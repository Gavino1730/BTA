...
async function seedCoachAccountAndRoster(api: APIRequestContext): Promise<SeedResult> {
  ...
  const schoolId = `concurrent-corr-${seed}`;
  const coachEmail = `concurrent.${seed}@bta.local`;
  ...
  const registerRes = await api.post(`${API_BASE}/api/auth/register`, {
    ...
    data: {
      fullName: "Concurrent Correction Coach",
      email: coachEmail,
      password,
  ...
  const onboardingRes = await api.post(`${API_BASE}/api/onboarding/complete`, {
    ...
    data: {
      ...
      schoolName: "E2E High School",
      coachName: "Concurrent Correction Coach",
      coachEmail,
      teamName: "E2E Eagles",
      abbreviation: "E2E",
      season: "2026",
...
test("coach correction can apply while operator continues posting", async ({ browser, request }) => {
  ...
  try {
    ...
    await coachPage.getByPlaceholder("e.g. Opponent").fill("Concurrency Opponent");
    ...
    const initialEvents = await fetchGameEvents(request, gameId, seed.token, seed.schoolId);

    const targetEvent = initialEvents[0];

    const [postResult, deleteResult] = await Promise.all([
      (async () => {
        await recordMadeTwoPoint(operatorPage);
        return true;
      })(),
      request.delete(`${API_BASE}/api/games/${encodeURIComponent(gameId)}/events/${encodeURIComponent(targetEvent.id)}`, {
        headers: {
          Authorization: `Bearer ${seed.token}`,
          "x-school-id": seed.schoolId,
          "x-event-sequence": String(targetEvent.sequence),
        },
      }),
    ]);
    ...
    await expect.poll(async () => {
      const events = await fetchGameEvents(request, gameId, seed.token, seed.schoolId);
      return events.length;
    }, { timeout: 20_000 }).toBeGreaterThanOrEqual(initialEvents.length);

    const finalEvents = await fetchGameEvents(request, gameId, seed.token, seed.schoolId);

    expect(finalEvents.some((event) => event.id === targetEvent.id)).toBe(false);
