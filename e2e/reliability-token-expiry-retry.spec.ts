...
async function seedCoachAccountAndRoster(api: APIRequestContext): Promise<SeedResult> {
  ...
  const schoolId = `token-refresh-${seed}`;
  const coachEmail = `token.${seed}@bta.local`;
...
test("queued flush recovers from mid-game token expiry and retries with refreshed credentials", async ({ browser, request }) => {
  ...
  try {
    ...
    await coachPage.getByPlaceholder("e.g. Opponent").fill("Token Retry Opponent");
    ...
    const eventsBeforeExpiry = await fetchGameEvents(request, gameId, seed.token, seed.schoolId);

    const staleToken = `bta.expired-token-${Date.now()}`;
    await operatorPage.evaluate(({ stale, key }) => {
      const raw = window.localStorage.getItem(key);
      if (!raw) {
        return;
      }
      const parsed = JSON.parse(raw) as {
        gameSetup?: { apiKey?: string };
      };
      parsed.gameSetup = {
        ...(parsed.gameSetup ?? {}),
        apiKey: stale,
    ...
    }, { stale: staleToken, key: ROSTER_STORAGE_KEY });
    ...
    operatorPage.on("request", (req) => {
      ...
      if (!req.url().includes(`/api/games/${encodeURIComponent(gameId)}/events`)) {
        return;
      }
      const headers = req.headers();
      authHeadersSeen.push(headers.authorization ?? headers["x-api-key"] ?? "");
    });

    let forcedStaleUnauthorizedCount = 0;
    await operatorPage.route("**/api/games/**/events", async (route) => {
      ...
      if (authHeader.includes(staleToken)) {
        forcedStaleUnauthorizedCount += 1;
        await route.fulfill({
          status: 401,
          contentType: "application/json",
          body: JSON.stringify({ error: "simulated_expired_operator_token" }),
        });
        return;
      }
    ...
    await expect.poll(() => {
      return authHeadersSeen.some((header) => header.includes(staleToken));
    }, { timeout: 15_000 }).toBe(true);

    await expect.poll(() => {
      return forcedStaleUnauthorizedCount > 0;
    }, { timeout: 15_000 }).toBe(true);
    ...
    }, { timeout: 30_000 }).toBeGreaterThanOrEqual(eventsBeforeExpiry.length + 3);
