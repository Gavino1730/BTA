...
- **Run Detection (Rule 7 rework)**: Replaced "last 10 events" sliding window with `computeUninterruptedRun` — walks backwards through scoring events and counts consecutive points for one team until the other scores. Alerts are now "opponent 8-0 run (started Q2)" and "we're on a 10-0 run" style.
- **Clutch Rule Entry Guard (Rule 5)**: Added window-entry guard — clutch alert fires only once when clock first crosses below 120s in Q4/OT, not on every subsequent event in the window.
- **Sub Suggestion Intelligence (Rule 9)**: Checks roster availability via `context.rosterPlayers`; when bench is empty and a player has 4 fouls, suggests a timeout for scheme adjustment instead of an impossible sub.
- **Possession Inference (Rule 13)**: Rule 13 infers possessions from shot attempts + turnovers when no explicit `possession_start` events exist, so PPP efficiency gap fires reliably without manual possession tracking.
...
- **PPP Possession Inference**: Scoreboard PPP now falls back to inferred possessions (FGA + turnovers) when operators don't log `possession_start` events, so PPP columns always populate.
...
- **Historical Context TTL at Period Transitions**: `refreshGameAiInsights` and `requestAiChatResponse` now force a historical context refresh on `period_transition` events in addition to the time-based TTL interval.
...
- **Optimistic Local State**: `postEvent` adds events to `pendingEvents` immediately before the server round-trip; display state (scores, totals, event feed) derives from both `submittedEvents` and `pendingEvents` so UI updates instantly on tap.
...
### 4. Coach Dashboard — Insight Age Display
