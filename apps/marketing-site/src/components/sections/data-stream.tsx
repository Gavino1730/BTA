...
export function DataStream(): JSX.Element {
  return (
    <section className="mx-auto w-[min(1200px,92vw)] py-8">
      <div className="relative overflow-hidden rounded-2xl border border-[var(--border-soft)] bg-[var(--panel-1)] px-4 py-4">
        ...
        >
          {Array.from({ length: 2 }).flatMap((_, repeatIdx) =>
            streamItems.map((item, idx) => (
              <span
                key={`${repeatIdx}-${item}`}
                className="inline-flex items-center gap-2 rounded-full border border-[var(--border-soft)] bg-[var(--panel-2)] px-3 py-1.5 text-xs text-[var(--text-secondary)]"
              >
                <span
