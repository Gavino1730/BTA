...
REALTIME_API_BASE = os.environ.get("REALTIME_API_BASE", "http://localhost:4000").rstrip("/")
...
def _build_api_error_response(status_code: int, message: str):
    payload = {
        "error": message,
        "status": status_code,
    }
...
def handle_http_exception(error):
    ...
    return _build_api_error_response(error.code or 500, message)
...
def handle_api_exception(error):
    ...
    return _build_api_error_response(500, "Internal server error")
...
# =============================================================================
# Data Ingestion  (Operator App → Stats Dashboard)
# =============================================================================
...
def _recompute_season_stats(stats: dict, roster: list = None) -> None:
    """Rebuild season_player_stats, season_team_stats, and player_game_logs
    from scratch by iterating all games[]. Also includes all roster players
    with zero stats if they don't appear in game data. Modifies stats in-place."""
    ...
    for game in games:
        ...
        for ps in game.get("player_stats", []):
            ...
            sp["games"] += 1
            sp["pts"] += ps.get("pts", 0)
            # Support both operator format (fg_made/fg_att) and legacy format (fg/fga)
    ...
    # Also include all roster players with zero stats (in case season hasn't started or they haven't played)
    if roster:
        for roster_player in roster:
            player_name = (roster_player.get("name") or "").strip()
            if player_name and player_name not in season_player_stats:
                season_player_stats[player_name] = {
                    "name": player_name,
                    "number": roster_player.get("number"),
...
def ingest_game():
    """Receive a completed game from the operator app and persist it."""
    ...
    with reload_lock:
        try:
            ...
            # Sync roster.json from the full roster field sent by the operator app.
            ...
            try:
                ...
                if incoming_roster:
                    # Preferred path: operator app sends full player objects keyed by jersey
                    existing_by_number = {
                        p.get("number"): p
...
# =============================================================================
# Team & Roster Management API
# =============================================================================


def _get_roster_data() -> dict:
...
def _sync_to_realtime_api(roster_data: dict) -> None:
    """Sync roster changes to realtime API."""
    _sync_roster_to_realtime(roster_data)


@app.route("/api/teams", methods=["GET"])
def api_get_teams():
    """Get all teams (returns roster data as single team for now)."""
    roster = _get_roster_data()
    team_name = roster.get("team", "Team")
...
def api_create_team():
    """Create a new team (updates the primary roster)."""
    payload = request.get_json(force=True, silent=True)
    if not payload or not isinstance(payload, dict):
    ...
    try:
        ...
        return jsonify({
            ...
            "team": {"id": "primary-team", "name": name}
        }), 201
    except Exception as e:
...
def api_get_players():
    ...
    for player_name, stats in data.season_player_stats.items():
        roster_info = roster_players.get(player_name)
        player_data = {
            "name": player_name,
            **stats,
...
@app.route("/api/player/<player_name>", methods=["POST"])
def api_add_player(player_name):
...
def api_delete_player_route(player_name):
    ...
    before = len(roster)
    roster_data["roster"] = [p for p in roster if p.get("name") != player_name]
    
    if len(roster_data["roster"]) == before:
        return jsonify({"error": "Player not found"}), 404
    
    try:
        _save_roster_data(roster_data)
        _sync_to_realtime_api(roster_data)
        return jsonify({"message": "Player deleted successfully", "player": player_name})
    except Exception as e:
        logger.error(f"Player deletion error: {e}")
...
def _ai_service_error_response(message: str):
    return jsonify({"error": message, "code": "ai_unavailable"}), 503


@app.route("/api/ai/chat", methods=["POST"])
def ai_chat():
    """Conversational AI endpoint for stats Q&A."""
    try:
        payload = request.get_json(force=True, silent=True)
        if not payload or not isinstance(payload, dict):
            return jsonify({"error": "Invalid or missing JSON payload"}), 400
        ...
        return _ai_service_error_response(str(e))
    except Exception as e:
...
def ai_analyze():
    """General-purpose AI analysis endpoint for stats dashboard workflows."""
    try:
        payload = request.get_json(force=True, silent=True)
        if not payload or not isinstance(payload, dict):
            return jsonify({"error": "Invalid or missing JSON payload"}), 400
        ...
        analysis_type = str(payload.get("type") or "general").strip().lower()
